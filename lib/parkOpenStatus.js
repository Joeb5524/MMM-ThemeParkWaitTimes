...
function pickTodayOperatingWindows(schedule, timezone, nowInParkTz) {
    ...
    return (Array.isArray(schedule) ? schedule : [])
        ...
        .map((entry) => {
            ...
            if (!open || !close || !open.isValid || !close.isValid) return null;
...
function pickNextOpening(schedule, timezone, nowInParkTz) {
    const futureWindows = (Array.isArray(schedule) ? schedule : [])
        .filter((entry) => entry && entry.type === "OPERATING" && entry.openingTime && entry.closingTime)
        .map((entry) => {
            const open = safeDateTimeFromISO(entry.openingTime, timezone);
            const close = safeDateTimeFromISO(entry.closingTime, timezone);
            if (!open || !close || !open.isValid || !close.isValid) return null;
...
function computeParkOpenState({
    ...
    const todayWindows = pickTodayOperatingWindows(schedule, resolvedTimezone, nowInParkTz);
    ...
    for (const window of todayWindows) {
        const effectiveOpen = window.openingTime.minus({ minutes: graceBeforeOpenMins });
        const effectiveClose = window.closingTime.plus({ minutes: graceAfterCloseMins });
